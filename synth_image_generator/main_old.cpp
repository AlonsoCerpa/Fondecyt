...
int num_piezas = 3;
int num_backgrounds = 6;
int num_transf = 175;
std::string dir_images = "images";
std::string dir_backgrounds = "best_backgrounds";
	...
	for (int j = 0; j < num_piezas; ++j)
	{
		for (int i = 0; i < num_backgrounds; ++i)
		{
			...
			std::string name_model_back =
				base_path + "//" +
				//"./" + 
				dir_backgrounds + "/pieza" + std::to_string(j) + "/back_" + std::to_string(i) + ".obj";
			models_backgrounds[j][i] = new Model((GLchar*)name_model_back.c_str());
		}
	}

	glm::mat4 model_background(1.0f);
	model_background = glm::scale(model_background, glm::vec3(4.0f, 4.0f, 1.0f));
